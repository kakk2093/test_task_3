...
public class RGCController : MonoBehaviour
{
    ...
    private void SetRGB()
    {
        if (_isCube)
        {
            _meshRenderer.material.color =new Color(rValue, gValue, bValue);
        }
        else
        {
            _image.color = new Color(rValue, gValue, bValue);
        }
    ...
    {
        rValue = _rVolueToConvert / 256;
        bValue = _bValueToConvert / 256;
        gValue = _gVovueToConvert / 256;
    }
